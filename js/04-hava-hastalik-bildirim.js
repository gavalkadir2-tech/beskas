...
function updateFinanceCatOptions(){
  const tpEl=document.getElementById('f-tp');const ctEl=document.getElementById('f-ct');
  if(!tpEl||!ctEl)return;
  const key=tpEl.value==='Gider'?'expense':'income';
  ctEl.innerHTML=catOpts(key);
}
...
const FD={
  ...
  <div class="form-section">
    ...
    <div class="fr3"><div class="fg"><label>Küpe No *</label><input id="f-tag" placeholder="TR001"/><div class="err-msg"></div></div><div class="fg"><label>İsim</label><input id="f-name" placeholder="Sümbül"/></div><div class="fg"><label>Tür *</label><select id="f-sp">${catOpts('species')}</select></div></div>
    ...
    <div class="fr3"><div class="fg"><label>Doğum Tarihi</label><input id="f-bd" type="date"/></div><div class="fg"><label>Renk / İşaret</label><input id="f-cl" placeholder="Siyah-beyaz"/></div><div class="fg"><label>Konum</label><input id="f-lc" list="f-lc-dl" placeholder="Ahır A"/>${catDatalist('f-lc-dl','locations')}</div></div>
